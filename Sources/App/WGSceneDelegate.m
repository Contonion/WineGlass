...
// Host-side blocking HTTPS download (NSURLSession). Lives in this always-linked
// app file so the engine's package-fetch hook can rely on it — a standalone new
// .m kept not getting compiled into the target (null-symbol 0x0 crash). See
// wg_native_download.h. Called from the engine's background thread (safe to block).
